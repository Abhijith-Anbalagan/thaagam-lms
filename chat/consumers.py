...
class ChatConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if not body or not receiver_id: return
        user    = self.scope['user']
        message = await self.save_message(user.pk, int(receiver_id), int(self.classroom_id), body)
        for grp in [f'chat_{self.classroom_id}_{user.pk}', f'chat_{self.classroom_id}_{receiver_id}']:
    ...
    def save_message(self, sender_id, receiver_id, classroom_id, body):
        ...
        return Message.objects.create(
            ...
            classroom_id=classroom_id, body=body,
        )
